...
| Método | Rota | Parâmetros | O que faz |
|---|---|---|---|
| `POST` | `/card/{id}/validate-move` | rota `id`, query `targetStageId` | Pre-check de movimentação, não altera estado. Ver [Mover card](../mover-card) |
| `PUT` | `/card/{id}/stage/{stageId}` | rota `id`, `stageId`, query `force` | Move o card para outra etapa do mesmo funil. Ver [Mover card](../mover-card) |
| `POST` | `/card/{id}/move-funnel` | rota `id`, body | Move um card aberto para outro funil. Ver [Trocar de funil](../trocar-de-funil) |
| `POST` | `/card/move-by-phone` | body | Move os cards de um contato identificado pelo telefone. Ver [Mover por telefone](../mover-por-telefone) |
| `POST` | `/card/bulk/move` | body | Move vários cards de uma vez. Ver [Mover em massa](../mover-cards-em-massa) |
