...
export async function buildProductMap({ frontRoot, backRoot, guides, actions, baseline }) {
  ...
  for (const file of await files(join(frontRoot, 'src'), ['.tsx', '.jsx'])) {
    ...
    for (const match of source.matchAll(/data-(tour|help)-id\s*=\s*(["'])([^"']+)\2/gu)) {
      const id = safe(match[3], /^[A-Za-z0-9_-]{1,80}$/u);
      if (id) markers.push({ kind: match[1], id });
      else pending.push(`expressão ou marcador inseguro: ${path}`);
    }
    if (/data-(?:tour|help)-id\s*=\s*\{/u.test(source)) pending.push(`marcador dinâmico: ${path}`);
    for (const match of source.matchAll(/<(?:button|Button)\b[^>]*>([^<{}]{1,100})<\/(?:button|Button)>/gu)) {
      const label = safe(match[1].trim());
  ...
  for (const file of await files(backRoot, ['.cs'])) {
    const source = await readFile(file, 'utf8');
    for (const match of source.matchAll(/^\s*\[Authorize(?:\s*\(\s*(?:(Policy|Roles)\s*=\s*)?["']([^"']+)["']\s*\))?\s*\]/gmu)) {
      const controller = relative(backRoot, file).split('/').at(-1).replace(/\.cs$/u, '');
      const raw = match[2] ? (match[1] === 'Roles' ? `${controller}:role:${match[2]}` : match[2]) : `${controller}:authenticated`;
      const name = safe(raw, /^[A-Za-z0-9_.:-]{1,100}$/u);
  ...
  const changes = [
    ...
    ...differences(baseline?.permissions, manifest.permissions, 'permission', (item) => item.name),
  ];
