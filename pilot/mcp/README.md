...
`npm run qa:guide-proof` usa o Chrome do QA existente (`CHROME_PATH`, se necessário). Sem conta de teste, roda somente a fixture local e imprime `pendente: conta de teste de homologação (Bruno)`. Em homologação, o relatório e os prints com texto e imagens ocultados ficam em `pilot/.guide-proof/`, ignorado pelo Git. Nunca copie essa pasta para `public/` ou para uma PR; no CI, só publique como artifact privado de retenção curta. Os roteiros de prova privados ficam em `pilot/scripts/guide-proof/roteiros/`; um passo sem roteiro fica `manual_required`.
...
Para concluir o aceite, a equipe precisa criar as duas contas fictícias, um departamento de teste chamado Demo e marcar os controles dos três guias no app de homologação. Até lá, a prova local não substitui a jornada real.
...
O registro integral das perguntas fica separado, em `CONVERSATIONS_FILE` (padrão `/tmp/ihelp-docs-conversations.jsonl`; no Railway, configurar `/data/claricia-conversations.jsonl`). Cada resposta grava pergunta original, resposta, classificação, origem, empresa válida do app e `eventId` que liga a avaliação. `CONVERSATIONS_RETENTION_DAYS` é opcional: sem ela não há limpeza. A tela interna fica em `/admin/claricia` e usa `FEEDBACK_ADMIN_TOKEN`; o navegador mantém a credencial apenas em `sessionStorage`. O arquivo contém dados pessoais enviados pelos clientes; mantenha o volume e a credencial restritos ao serviço interno.
...
- `criar_guia`: writer informa `guideId`, `topic`, `module` e `description`; recebe `planId`, `plan`, `questions` e `questionIds`. Na retomada reenvia o pedido original, esses três campos e `answers` na mesma ordem. O estado em `MCP_STATE_DIR/.guide-plans` guarda hashes do pedido, plano, respostas e guia anterior, ids das perguntas, ator, fonte e expiração; não guarda texto livre do pedido ou das perguntas. Qualquer diferença exige refazer o plano. Fontes alteradas também exigem novo plano; repetição das mesmas respostas devolve o mesmo draft. O resultado passa por `docs_submit_package` em modo `draft`, retorna `reviewRequired: true` e exige revisão humana para publicar.
...
O guia devolve `stateToken` assinado para cada passo. Avançar, escolher ramo ou concluir exige o token do passo atual; uma versão, passo ou token inválido oferece recomeçar ou falar com uma pessoa. **Reinício do serviço invalida guias em andamento, e a pessoa recomeça**: o token inclui um identificador aleatório da instância, mesmo quando `LOG_ID_KEY` é persistente.
...
## Release por Git (M5.29)

O workflow `deploy.yml` valida PRs sem publicar. Push em `main` publica staging e `workflow_dispatch` pode publicar staging ou production somente quando a variável de repositório `CLARICIA_DEPLOY_ENABLED=true`. Sem ela, o build valida e informa que o deploy está desativado. O job de cada ambiente usa GitHub Environment; configure aprovação obrigatória para `production` antes de ativar a flag. Deploy real depende do Bruno.

Configure `CLARICIA_DEPLOY_ENABLED` no repositório para ativar a publicação. Em cada GitHub Environment (`staging` e `production`), configure `NEXT_PUBLIC_ASSISTANT_URL` (HTTPS, terminada em `/assistant`), `CLARICIA_DOCS_ORIGIN` (origem HTTPS do site), `CLARICIA_DOCS_URL` (URL do site publicado), `RAILWAY_PROJECT_ID` e `RAILWAY_SERVICE_ID`. Em `staging`, configure também `RAILWAY_DOCS_SERVICE_ID`. `RAILWAY_TOKEN` fica apenas nos Environment secrets. O health é consultado em `/health` na mesma origem da URL embutida no artifact. No Railway, escolha `Dockerfile.mcp` para o serviço Claricia e `Dockerfile.docs` para o site staging, ambos com raiz de build em `pilot/`. Monte volume persistente em `/data` no serviço Claricia e configure `MCP_STATE_DIR=/data`; mantenha `ASSISTANT_BUDGET_FILE`, `FEEDBACK_FILE` e `SESSION_EVENTS_FILE` nesse volume. Configure `ASSISTANT_ALLOWED_ORIGINS` com as origens HTTPS exatas do staging e do GitHub Pages. As demais credenciais do MCP ficam só em variáveis privadas do Railway.

O build grava `release.json` público com SHA do commit, SHA do pacote imutável de guias e endereço público da Claricia; não coloque token nesses campos. O smoke falha se o endereço não estiver realmente no JS/HTML, se `/health` do serviço apontar para outra versão, ou se o site staging não servir o SHA esperado em `/ihelp-docs/release.json`. O site staging é publicado só depois do serviço compatível. O `/health` não expõe credenciais.

Para rollback, identifique no histórico do GitHub Actions a execução anterior com SHAs compatíveis. Restaure primeiro o deployment anterior do serviço Claricia e depois o deployment anterior do site staging no Railway; para GitHub Pages, reexecute o job de Pages daquela execução. Confira `/health` e `/ihelp-docs/release.json` contra o par de SHAs anterior. Não apague nem restaure `/data` automaticamente: o volume com drafts, audit, feedback e eventos sobrevive a restart e rollback de imagem. O pacote anterior permanece em `public/guides/<sha12>/`; confira também o `manifest.json` da versão restaurada. Faça backup do volume antes de uma recuperação que precise alterar estado.
