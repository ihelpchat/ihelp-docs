...
- `criar_guia`: writer informa `guideId`, `topic`, `module` e `description`; recebe `planId` e perguntas. Depois envia `planId` e `answers` na mesma ordem. O plano fica em `MCP_STATE_DIR/.guide-plans`, vinculado ao ator e aos SHAs das fontes. Fontes alteradas exigem novo plano; repetição das mesmas respostas devolve o mesmo draft. O resultado passa por `docs_submit_package` em modo `draft` e exige revisão humana para publicar.
