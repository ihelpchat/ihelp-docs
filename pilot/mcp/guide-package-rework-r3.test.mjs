...
import { glob, mkdtemp, mkdir, readFile, readdir, writeFile } from 'node:fs/promises';
...
const corpus = (await filesUnder(publishedRoot)).sort();
const inventory = (await Array.fromAsync(glob('**/*.mdx', { cwd: publishedRoot })))
  .map((path) => join(publishedRoot, path)).sort();
const checked = [];
const failures = {};
