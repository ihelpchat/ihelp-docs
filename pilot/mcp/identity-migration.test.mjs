...
async function start(extraEnv, shouldStart = true) {
  ...
  const child = spawn(process.execPath, [new URL('./http.mjs', import.meta.url).pathname], {
    env: { ...baseEnv, PORT: String(port), MCP_STATE_DIR: await mkdtemp(join(tmpdir(), 'm514-migration-')), ...extraEnv },
  ...
  for (let attempt = 0; attempt < 100; attempt++) {
    ...
    try {
      ...
      if (shouldStart) return { child, url, stderr: () => stderr };
      child.kill('SIGINT');
...
try {
  ...
  try {
    ...
    const write = await client.callTool({ name: 'docs_submit_package', arguments: { articles: [], mode: 'dry_run' } });
  ...
  assert.doesNotMatch(legacy.stderr(), legacyKey);
...
try {
  ...
  try {
    const withoutActor = await client.callTool({ name: 'docs_submit_package', arguments: { articles: [], mode: 'dry_run' } });
  ...
  assert.doesNotMatch(both.stderr(), legacyKey);
