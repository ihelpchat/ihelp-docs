...
try {
  const closeProbe = spawnSync(process.execPath, [join(source, 'scripts/visual/site-close.test.mjs')],
    { cwd: source, env: process.env, stdio: 'inherit' });
  if (closeProbe.error) throw closeProbe.error;
  if (closeProbe.status !== 0) process.exitCode = closeProbe.status ?? 1;
  else {
