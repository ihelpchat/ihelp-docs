...
export const screens = [
  {
    ...
    checks: [
      ...
      { id: 'nav-ia', only: 'desktop', design: up(t('Assistente de IA'), 'r.height >= 28'), app: '.ih-nav-link[href$="/assistente"], .ih-nav-link[href$="/assistente/"]', props: ['left', 'width', 'height', 'fontWeight'] },
      { id: 'nav-ajuda', only: 'desktop', design: up(t('Central de ajuda'), 'r.height >= 28'), app: '.ih-nav-link[href$="/docs"], .ih-nav-link[href$="/docs/"]', props: ['left', 'width', 'height'] },
      { id: 'lancador', design: up(t('Assistente de IA', 1), 'r.height >= 40'), app: '.ih-ai-launcher', props: [...layout, 'background', 'color', 'fontSize'] },
  ...
  },
  {
    ...
    checks: [
      ...
      { id: 'lateral-rotulo', only: 'desktop', design: `document.querySelector('aside > div > span')`, app: '.ih-side-kicker', props: ['left', 'top', ...type] },
      { id: 'recolher-tudo', only: 'desktop', design: t('Recolher tudo'), app: '.ih-side-all', props: ['right', 'top', 'fontSize', 'color'] },
      { id: 'grupo-1', only: 'desktop', design: up(t('Primeiros passos'), 'r.width > 200'), app: '.ih-sidebar .ih-side-group-toggle', props: ['left', 'top', 'width', 'height', 'fontSize', 'fontWeight', 'color'] },
      { id: 'grupo-1-contador', only: 'desktop', design: `__up(__byText('Primeiros passos'), (r) => r.width > 200).lastElementChild`, app: '.ih-sidebar .ih-side-count', props: ['right', 'fontSize'] },
      { id: 'grupo-recolhido', only: 'desktop', design: up(t('WhatsApp Business API'), 'r.width > 200'), app: '.ih-sidebar .ih-side-group:not([data-open]) .ih-side-group-toggle', props: ['left', 'width', 'height', 'fontSize', 'fontWeight'] },
  ...
  },
  {
    ...
    checks: [
      ...
      { id: 'grupo', only: 'desktop', design: up(t('Começar'), 'r.width > 200'), app: '.ih-sidebar-api .ih-side-group-toggle', props: ['left', 'top', 'width', 'height'] },
      { id: 'grupo-titulo', only: 'desktop', design: t('Começar'), app: '.ih-sidebar-api .ih-side-group-title', props: ['left', ...type] },
      { id: 'recolher-tudo', only: 'desktop', design: t('Recolher tudo'), app: '.ih-side-all', props: ['right', 'top', 'fontSize', 'color'] },
  ...
  },
  {
    ...
    design: [{ click: 'Referência da API' }, { click: 'Atendimentos' }, { click: 'Buscar por telefone' }, { click: 'B · Coluna única' }],
  ...
  },
  {
    ...
    checks: [
      ...
      { id: 'contador', only: 'desktop', design: up(t('18 guias publicados no Tango'), 'r.height >= 30'), app: '.ih-status-pill', props: ['right', 'height', 'background'] },
      { id: 'player', design: up(t('Iniciar no app'), 'r.height > 400'), app: '.ih-player', props: ['left', 'width', 'background'] },
      { id: 'lista-rotulo', only: 'desktop', design: t('Guias de Atendimento'), app: '.ih-guide-column .ih-eyebrow', props: ['left', ...type] },
      { id: 'guia-ativo', only: 'desktop', design: up(t('Responder e transferir um atendimento', 2), 'r.height >= 50'), app: '.ih-guide[aria-pressed="true"]', props: ['left', 'width', 'background'] },
      { id: 'modulos', only: 'desktop', design: `document.querySelector('aside')`, app: '.ih-tut-aside', props: ['left', 'top', 'width', 'background'] },
      { id: 'modulos-rotulo', only: 'desktop', design: t('Módulos'), app: '.ih-tut-aside .ih-side-kicker', props: ['left', 'top', ...type] },
      { id: 'modulos-filtro', only: 'desktop', design: up(`document.querySelector('input[placeholder="Filtrar guias"]')`, 'r.height >= 30'), app: '.ih-tut-filter', props: [...layout, 'background'] },
      { id: 'modulo-1', only: 'desktop', design: up(t('Primeiros passos'), 'r.width > 200'), app: '.ih-tut-aside .ih-side-group-toggle', props: ['left', 'top', 'width', 'height', 'fontSize', 'fontWeight'] },
    ],
  },
  {
    name: 'novidades',
  ...
  },
  {
    name: 'assistente',
    viewports: ['desktop', 'mobile'],
    // O protótipo desenha a tela do assistente também no celular, então ali posição e grade também contam.
    mobileLayout: true,
    app: '/assistente/',
    design: [{ click: 'Assistente de IA' }],
    checks: [
      { id: 'nav-ativo', only: 'desktop', design: up(t('Assistente de IA'), 'r.height >= 28'), app: '.ih-nav-link[data-active]', props: ['left', 'width', 'background', 'fontWeight'] },
      { id: 'icone', design: `document.querySelector('h1').previousElementSibling`, app: '.ih-ai-empty-icon', props: [...layout, 'background', 'borderColor'] },
      { id: 'titulo', design: 'document.querySelector("h1")', app: '.ih-ai-empty h1', props: ['left', 'top', ...type] },
      { id: 'abertura', design: 'document.querySelector("h1 + p")', app: '.ih-ai-empty > p', props: ['left', 'top', 'width', 'fontSize', 'color'] },
      { id: 'sugestao-1', design: up(t('Como transfiro um atendimento para outro departamento?'), 'r.height >= 60'), app: '.ih-ai-starters button', props: [...layout, 'background'] },
      { id: 'sugestao-selo', design: t('Ajuda'), app: '.ih-ai-starters .ih-pill', props: ['left', 'top', 'fontSize', 'background', 'color'] },
      { id: 'escopo-rotulo', design: t('Buscar em'), app: '.ih-ai-scopes > span', props: ['left', 'top', 'fontSize', 'color'] },
      { id: 'escopo-ativo', design: up(t('Tudo'), 'r.height >= 24'), app: '.ih-ai-scopes .ih-chip[aria-pressed="true"]', props: ['left', 'top', 'height', 'background', 'color'] },
      { id: 'campo', design: up(`document.querySelector('textarea')`, 'r.height >= 50'), app: '.ih-ai-composer', props: [...layout, 'background', 'borderColor'] },
      { id: 'enviar', design: `document.querySelector('textarea').parentElement.lastElementChild`, app: '.ih-ai-send', props: [...layout] },
      { id: 'aviso-ia', design: t('Respostas geradas por IA a partir da documentação. Confira as fontes antes de agir.'), app: '.ih-ai-hint > span', props: ['left', 'top', 'fontSize'] },
      { id: 'painel', only: 'desktop', design: up(t('Fontes da resposta'), 'r.height > 500'), app: '.ih-ai-panel', props: ['left', 'top', 'width', 'background'] },
      { id: 'painel-vazio', only: 'desktop', design: up(t('Quando o assistente responder, os artigos, tutoriais e endpoints usados aparecem aqui para você conferir.'), 'r.height > 60'), app: '.ih-ai-panel-empty', props: [...layout, 'borderColor'] },
    ],
  },
  {
    name: 'painel',
    ...
    mobileLayout: true,
    app: '/docs/sobre-o-sistema/atendimento/',
    appClicks: ['.ih-ai-launcher'],
    design: [{ click: 'Central de ajuda' }, { click: 'Assistente de IA', nth: 1 }],
    checks: [
      { id: 'caixa', design: up(t('Como posso ajudar?'), 'getComputedStyle(n).position === "fixed"'), app: '.ih-ai-drawer', props: [...layout, 'background', 'borderRadius'] },
      { id: 'titulo', design: t('Assistente de IA', 1), app: '.ih-ai-drawer-title', props: ['left', 'top', ...type] },
      { id: 'contexto', design: up(t('Você está em'), 'r.width > 300'), app: '.ih-ai-drawer-context', props: ['left', 'top', 'height', 'background'] },
      { id: 'contexto-pagina', design: up(t('Central de ajuda › Atendimento'), 'getComputedStyle(n).borderTopWidth !== "0px"'), app: '.ih-ai-drawer-context strong', props: ['left', 'fontSize', 'fontWeight'] },
      { id: 'vazio-titulo', design: t('Como posso ajudar?'), app: '.ih-ai-drawer-empty-title', props: ['left', 'top', ...type] },
      { id: 'sugestao-1', design: up(t('Resuma este artigo em 3 pontos'), 'r.height >= 36'), app: '.ih-ai-drawer-empty button', props: [...layout] },
      { id: 'campo', design: up(`[...document.querySelectorAll('textarea')].at(-1)`, 'r.height >= 40'), app: '.ih-ai-drawer .ih-ai-composer', props: [...layout, 'borderColor'] },
    ],
  },
  {
    name: 'busca',
